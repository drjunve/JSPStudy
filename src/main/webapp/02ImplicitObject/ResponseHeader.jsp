...
즉 9시간이 빠르므로 그만큼을 더해줘야 정상적인 날짜가 출력된다. 만약 09시 이전의
시간으로 세팅되면 하루전의 날짜가 출력된다.
*/
SimpleDateFormat s = new SimpleDateFormat("yyyy-MM-dd HH:mm");
//초단위로 변경된 시간을 long타입의 변수에 저장한다.
long add_date = s.parse(request.getParameter("add_date")).getTime();
out.println("add_date="+add_date);
...
//정수형식 추가. 동일한 헤더명으로 2개의 값을 추가한다. ^addIntHeader은 8282, 1004 두개가 나와야 정상
response.addIntHeader("myNumber", add_int);
response.addIntHeader("myNumber", 1004); // 추가
//기존의 응답레더를 '안중근'으로 수정한다. ^myName은 수정된 값 안중근 하나만 나와야 정상
