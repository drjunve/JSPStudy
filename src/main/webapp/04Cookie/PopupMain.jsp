<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
/* 해당 페이지에서는 가장 먼저 팝업창 오픈에 관련된 쿠키가 있는지 먼저
확인한 후 팝업창을 처리해야한다. */
//팝업창 오픈 여부 판단을 위한 변수
String popupMode = "on"; 
    ...
    $('#closeBtn').click(function() {
    	...
    	$.ajax({
    		...
    		success : function(resData) {
    			/* 콜백데이터에 지시어 때문에 남게되는 공백때문에
    			불필요한 새로고침이 일어날 수 있다. 따라서 공백을
    			제거해야 정상적으로 동작하게 된다.*/
    			if(resData) {
    				console.log('있다')
    			}
    			...
    			/* 만약 콜백데이터가 있다면 페이지를 새로고침한다. F5를 누르는 것과 동일하다. */
    			//if (resDate != '') location.reload();
    		}
    	});
    });
});
...
<%
	...
	/* 만약 변수값이 on이 아니라면 아래 팝업창을 위한 HTML코드는
	물리적으로 삽입되지 않는다. 즉, 쿠키의 상태에 따라 페이지가 동적으로 변경된다.*/
