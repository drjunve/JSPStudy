...
//세션 영역에 저장된 회원 아이디를 가져와서 문자열로 변환한다.
...
수저페이지로 진입할 수 있다. 따라서 수정페이지 자체에서도 작성자
...
<!-- 수정페이지는 일반적으로 쓰기페이지를 복사해서 제작하게돼므로 
...
<input type="hid(input태그에는정확한속성명이아니면무조건text속성으로된다.)den" name="num" value="<%= dto.getNum() %>" />
