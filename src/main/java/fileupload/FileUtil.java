...
public class FileUtil {
	...
	throws ServletException, IOException {
		...
		/* 파일 첨부를 위한 <input> 태그의 name속성 값을 이용해서 part객체를
		...
		/* 앞에서 split()한 결과중 인덱스1은 파일명이 된다ㅏ. 여기서 더블쿼테이션을 제거하면
		 * 순수한 파일명만 남는다. replae를 통해 제거한다. 이때 더블쿼테이션을 제거할 문자열로
		 * 사용하기 위해 이스케이프 시퀀스를 붙여줘야 한다. */
		String originalFileName = phArr[1].trim().replace("\"", "");
	
		/* 전송된 파일이 있는 경우라면 디렉토리에 파일을 저장한다. 이때
		 * write() 메서드를 사용한다.
		 * File.separator : 운영체제(OS)마다 경로를 쵸시하는 기호가 다르므로
		 * 해당 Os예 맞는것을 자동을 기술해준다. */
	...
	public static String renameFile(String sDirectory, String fileName) {
		/*파일명에서 확장자를 잘라내기 위해 뒤에서부터 .이 있는 위치를
